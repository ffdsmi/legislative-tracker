 ...
 * @param {function} [onProgress] - Optional callback for streaming progress updates
 */
export async function ingestState(workspaceId, state, limit = 20, onProgress = null) {
    ...
    if (activeDataset && activeDataset.dataset_hash) {
      if (datasetTracker[`${state}_${currentSession.session_id}`] !== activeDataset.dataset_hash) {
        ...
        if (datasetPayload && datasetPayload.zip) {
          ...
          // Pre-cache extraction tracking for speed, process texts after bills just in case of constraints
          ...
          for (const entry of billEntries) {
            ...
            } catch(e) { /* ignore single parsing error */ }
          }
          ...
          for (const entry of textEntries) {
            try {
              ...
              if (data && data.text) {
                ...
                await saveTextVersion(workspaceId, textInfo.bill_id, textInfo.doc_id, plainText, {
                  date: textInfo.date,
          ...
          console.log(`[LegiScan] Bulk Extraction complete: mapped ${bulkExtracted} bills locally, parsed ${textExtracted} text documents.`);
          // Save active tracker
  ...
  if (masterBills && Array.isArray(masterBills)) {
    ...
    const totalBills = toCheck.length;
    let index = 0;

    for (const billSummary of toCheck) {
      index++;
      
      // Fire progress event if provided
      if (onProgress) {
        onProgress({ 
          current: index, 
          total: totalBills, 
          message: `Checking ${billSummary.bill_number}...`,
          metrics: { updated, skipped, limit }
        });
      }
      ...
      // Hard Limit Exit - Guard 30,000 API Quota Constraint!
      if (updated >= limit) {
        console.log(`[LegiScan] API Loop Limit Hit! Maxed out at ${limit} API-incurring bill fetches.`);
        break; 
      }
      ...
      try {
        ...
        // Count as "updated" (consumed API limit space)
        updated++;
      } catch (err) {
